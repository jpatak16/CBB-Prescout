...
opp_kp = gsub(" State", " St.", opp)
opp_kp2 = gsub("\\.", "", opp_kp)
opp_pos = kp_team_depth_chart(opp_kp, year = year)
opp_pg = opp_pos %>% select(first = pg_player_first_name, last = pg_player_last_name, min_pct = pg_min_pct, '#' = pg_number) %>% mutate(pos="PG") %>% filter(!is.na(first))
opp_sg = opp_pos %>% select(first = sg_player_first_name, last = sg_player_last_name, min_pct = sg_min_pct, '#' = sg_number) %>% mutate(pos="SG") %>% filter(!is.na(first))
opp_sf = opp_pos %>% select(first = sf_player_first_name, last = sf_player_last_name, min_pct = sf_min_pct, '#' = sf_number) %>% mutate(pos="SF") %>% filter(!is.na(first))
opp_pf = opp_pos %>% select(first = pf_player_first_name, last = pf_player_last_name, min_pct = pf_min_pct, '#' = pf_number) %>% mutate(pos="PF") %>% filter(!is.na(first))
opp_c = opp_pos %>% select(first = c_player_first_name, last = c_player_last_name, min_pct = c_min_pct, '#' = c_number) %>% mutate(pos="C") %>% filter(!is.na(first))
...
uconn = ifelse(opp=='Connecticut', "UConn", opp)
lastGdate = kp_team_schedule(opp_kp, year=year) %>% filter(is.na(pre_wp)) %>% arrange(desc(date)) %>% .[[1,18]]
opponentGID = espn_mbb_scoreboard(lastGdate) %>% filter(home_team_location == opp | away_team_location == opp | home_team_location == uconn | away_team_location == uconn) %>% .[[1,6]]
lastGstarters = espn_mbb_player_box(opponentGID) %>% filter(starter==TRUE) %>% filter(team_short_display_name==opp | team_short_display_name==uconn | team_short_display_name==opp_kp | team_short_display_name==opp_kp2) %>% 
...
PPtable_raw = SRopponentTables[[1]] %>% mutate(team=opp) %>%
...
headshot_urls = headshot_urls_db %>%
  separate(Player, into = c("first","last"), extra = "drop", sep = "[^\\w']") %>%
  mutate(last = str_to_title(last))

#join headshots to PPtable
PPtable_raw2 = left_join(PPtable_raw, headshot_urls, by=c("first", "last", "team"="Team"))
